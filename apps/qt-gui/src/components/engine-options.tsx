...
export function EngineOptions() {
  ...
  return (
    <section aria-labelledby="engine-options-title">
      ...
      <div className="grid min-h-12 grid-cols-[1fr_150px] items-center gap-3 border-b border-border/70 py-2">
        <Label htmlFor="mode" className="text-xs font-medium">Chế độ dịch</Label>
        <Controller
          name="mode"
          control={control}
          render={({ field }) => (
            <Select value={field.value} onValueChange={field.onChange}>
              <SelectTrigger id="mode" className="h-8 w-full text-xs">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="hanviet">Hán Việt</SelectItem>
                <SelectItem value="vietphrase">VietPhrase</SelectItem>
                <SelectItem value="vietphrase-one">Một nghĩa</SelectItem>
              </SelectContent>
            </Select>
          )}
        />
