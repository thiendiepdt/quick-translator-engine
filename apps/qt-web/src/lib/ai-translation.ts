...
const CHECK_RULES: Array<[RegExp, string, GenreSetting?]> = [
  ...
  [/(^|[.!?]\s+|[【(])["“']?(?:but|and|the|in|on|at|from|with)\b/i, "Từ nối tiếng Anh lọt vào bản dịch → dịch sang tiếng Việt hoặc chỉ giữ khi có căn cứ"],
  ...
  [/(?<!\p{L})đắc được(?!\p{L})/iu, "得到 / 深得 → được / nhận được / học được, không \"đắc được\""],
  [/(?<!\p{L})đắc thủ(?!\p{L})/iu, "得手 → thành công / ra tay trót lọt, không \"đắc thủ\""],
  [/(?<!\p{L})thu hoạch được(?!\p{L})/iu, "有收获 → thu được / tìm được gì, không \"thu hoạch được\""],
  [/(?<!\p{L})có biệt(?!\p{L})(?!\s*(?:hiệu|xưng|danh))/iu, "有别 → hữu biệt / khác biệt, không \"có biệt\""],
