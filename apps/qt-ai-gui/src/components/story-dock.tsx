...
export function StoryDock() {
  ...
  const entries = useMemo(() => dockEntries({ root, sessions, roots, names, progress }), [root, sessions, roots, names, progress]);
  ...
  return (
    <div className="flex flex-col items-center gap-1 border-t pt-2" role="list" aria-label="Truyện đang mở và đang dịch">
      {entries.map((item) => {
          ...
          <Tooltip key={item.root}>
            <TooltipTrigger asChild>
              <button
                ...
                className={cn(
                  "relative flex size-9 items-center justify-center rounded-lg border text-xs font-semibold transition-colors",
                  item.current
              ...
              >
                <span className={cn("flex size-7 items-center justify-center rounded-md", item.running && "bg-card")}>
      ...
      <Tooltip>
        ...
        <TooltipContent side="right">Chuyển truyện (Ctrl+K)</TooltipContent>
      </Tooltip>
      <StorySwitcherDialog
        open={switcherOpen}
